...
def cleandata(dataset, keepcolumns=['Country Name', '1990', '2015'], value_variables=['1990', '2015']):
    ...
    Args:
        ...
        keepcolumns: columns to be kept
        value_variables: variables to get values from
...
def return_figures():
    ...
    for country in countrylist:
        ...
        text = []
        for country_name, year in zip(country_label, year):
            text.append(str(country_name) + ' ' + str(year))
    ...
    graph_five.append(go.Bar(x=df_five.country.tolist(), y=df_five.ruralpopulation.tolist()))
